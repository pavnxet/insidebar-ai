...
async function init() {
  ...
  window.addEventListener('beforeunload', () => {
    try {
      browser.runtime.sendMessage({ action: 'sidePanelClosed', payload: {} });
    } catch (error) {
...
async function switchProvider(providerId) {
  ...
  await browser.storage.sync.set({ lastSelectedProvider: providerId });
      ...
      try {
        if (message.action === 'switchProvider') {
          ...
          if (message.payload?.selectedText) {
            ...
            const settings = await browser.storage.sync.get({ autoPasteClipboard: false });
...
async function renderPromptList(prompts = null) {
  ...
  setHTML(listContainer, prompts.map(prompt => `
...
function renderQuickAccessSection(containerId, prompts, emptyMessage) {
  ...
  if (prompts.length === 0) {
    setHTML(container, `<div class="quick-access-empty">No ${emptyMessage} prompts yet</div>`);
    return;
  }

  setHTML(container, prompts.map(prompt => {
...
async function dismissEdgeShortcutReminder() {
  try {
    await browser.storage.sync.set({ [EDGE_SHORTCUT_STORAGE_KEY]: true });
  } catch (error) {
...
function openBrowserShortcutSettings(browser) {
  ...
  try {
    ...
    console.warn('Unable to open shortcut settings via browser.tabs, falling back to window.open', error);
...
async function updateWorkspaceProviderSelector() {
  ...
  setHTML(popup, enabledProviders.map(provider => `
...
async function renderConversationList(conversations = null) {
  ...
  setHTML(listContainer, conversations.map(conv => {
...
async function viewConversation(id) {
  ...
  if (typeof marked !== 'undefined' && conversation.content) {
    try {
      ...
      setHTML(contentEl, sanitizeHtml(htmlContent));
  ...
  setHTML(providerEl, `<strong>Provider:</strong> ${providerIconHtml}${escapeHtml(conversation.provider)}`);
  ...
  if (conversationUrl) {
    setHTML(timestampEl, `<strong>Date:</strong> <a href="${escapeHtml(conversationUrl)}" target="_blank" class="conversation-link" title="Open original conversation">${date}</a>`);
  } else {
    setHTML(timestampEl, `<strong>Date:</strong> ${date}`);
  }

  const tagsEl = document.getElementById('view-conversation-tags');
  if (conversation.tags.length > 0) {
    setHTML(tagsEl, `<strong>Tags:</strong> ${conversation.tags.map(tag => `<span class="conversation-tag">${escapeHtml(tag)}</span>`).join(' ')}`);
  ...
  if (conversation.notes && !conversation.notes.startsWith('Extracted from: ')) {
    setHTML(notesEl, `<strong>Notes:</strong> ${escapeHtml(conversation.notes)}`);
