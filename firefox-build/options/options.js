...
function openShortcutSettings(browserOverride) {
  ...
  try {
    ...
    // Fallback to window.open if browser.tabs unavailable
...
async function renderProviderList() {
  ...
  setHTML(listContainer, PROVIDERS.map(provider => {
...
async function toggleProvider(providerId) {
  ...
  if (enabledProviders.includes(providerId)) {
    ...
    const lastSelected = await browser.storage.sync.get({ lastSelectedProvider: null });
    if (lastSelected.lastSelectedProvider === providerId) {
      await browser.storage.sync.set({ lastSelectedProvider: null });
    }
  } else {
    // Enable
...
async function loadLibraryCount() {
  ...
  try {
    const response = await fetch(browser.runtime.getURL('data/prompt-libraries/default-prompts.json'));
...
async function performVersionCheck() {
  ...
  try {
    ...
    if (result.error) {
      ...
      setHTML(statusDiv, t('msgUpdateStatusAvailable', [latest, date, current, message]));
