...
async function loadShortcutSetting() {
  try {
    const result = await browser.storage.sync.get(DEFAULT_SHORTCUT_SETTING);
...
async function toggleSidePanel(windowId, action = null) {
  ...
  if (!isOpen) {
    ...
    try {
      ...
      // Silently fail - sidebar may not be available
    }
  } else {
    // Close the sidebar by sending message to sidebar
    try {
...
async function configureActionBehavior() {
  // Firefox handles sidebar behavior through sidebar_action manifest key;
  // no runtime API configuration required.
}
...
browser.contextMenus.onClicked.addListener(async (info, tab) => {
  try {
    ...
    if (info.menuItemId.startsWith('provider-')) {
      ...
      const settings = await browser.storage.sync.get({ sourceUrlPlacement: 'end' });
    ...
    if (!isOpen) {
      ...
      try {
        ...
        if (sidebarResponse && sidebarResponse.hasFocus) {
          ...
          if (tab && tab.id) {
            try {
              await browser.tabs.sendMessage(tab.id, { action: 'takeFocus' });
            } catch (error) {
